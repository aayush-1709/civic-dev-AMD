...
async function generateReportWithAI({ clusters, complaints }) {
  ...
  const fallback = {
    report_title: 'AI Civic Intelligence Report',
  ...
  try {
    ...
    const prompt = [
      'You are generating a detailed civic intelligence report from full database data.',
...
async function answerCivicQueryWithAI({ question, complaints, clusters }) {
  ...
  const answerPrompt = [
    'You are a civic intelligence chatbot.',
